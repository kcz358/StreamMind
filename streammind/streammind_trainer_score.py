...
class StreamMindTrainer(Trainer):
    ...
    def _get_train_sampler(self, train_dataset=None) -> Optional[torch.utils.data.Sampler]:
        # transformers 4.x: no arg; transformers >=5.x: passes the dataset.
        # Keep our original behavior of inspecting self.train_dataset.
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], num_items_in_batch=None) -> torch.Tensor:
        ...
        # HF 5.x compute_loss may return the raw ModelOutput when labels/label_smoother
        # are absent; unwrap to the scalar loss tensor for deepspeed.backward().
        if hasattr(loss, "loss") and not isinstance(loss, torch.Tensor):
            loss = loss.loss
