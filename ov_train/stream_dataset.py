...
import math
from ov_train.soccer_data import (
    extract_video_half,
    trans_video_2_json,
    find_video_files,
    preprocess_qwen3_soccer as preprocess_llama_2_soccer,
    preprocess_llama_2_soccer_cls,
)
...
class LazySupervisedDataset(Dataset):
    ...
    def process_soccer_video(self, start_timestamp, end_timestamp, processor, video_path, cur_fps=2, video_backend="frames"):
        """Read a [start, end] segment of a soccer half at cur_fps via decord,
        resize+normalize each frame using OneVision's image_processor.

        Returns [T, 3, H, W] bf16 tensor ready for the OV2 vision tower, or
        ``None`` on failure (invalid timestamps, decord open failure, etc.).

        NOTE: the original ``expand2square`` aspect-preserving square pad is
        replaced by a plain bicubic resize to ``OV_FRAME_SIZE x OV_FRAME_SIZE``
        (default 448). Soccer 224p frames are 16:9, so this distorts; we accept
        that distortion in exchange for a fixed canvas required by the OneVision
        Qwen2VL-style flattened-patch path. To restore aspect-preserving behaviour
        ...
        if end_frame is None:
            return None
        frame_id_list = get_index(start_frame=start_frame, end_frame=end_frame, video_fps=video_fps,
                                  max_frame=duration, cur_fps=cur_fps)
        video_data = decord_vr.get_batch(frame_id_list).asnumpy()  # [T, H, W, 3] uint8
        if len(video_data) == 0:
            return None

        target_size = int(os.environ.get("OV_FRAME_SIZE", "448"))
        if target_size % 28 != 0:
            raise ValueError(f"OV_FRAME_SIZE={target_size} must be divisible by 28 (patch_size*spatial_merge_size)")

        imgs = [Image.fromarray(arr).convert("RGB").resize((target_size, target_size), Image.BICUBIC) for arr in video_data]
        arr = np.stack([np.asarray(im, dtype=np.float32) / 255.0 for im in imgs], axis=0)  # [T, H, W, 3]
        arr = arr.transpose(0, 3, 1, 2)  # [T, 3, H, W]
        mean = np.asarray(processor.image_mean, dtype=np.float32).reshape(1, 3, 1, 1)
        std = np.asarray(processor.image_std, dtype=np.float32).reshape(1, 3, 1, 1)
        arr = (arr - mean) / std
        video = torch.from_numpy(arr).to(torch.bfloat16)
        return video
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if self.soccer_dataset:
            ...
            for idx, start in enumerate(start_timestamp): 
                video = self.process_soccer_video(video_path=video_path, start_timestamp =start, end_timestamp = timestamp[idx], processor=video_processor,cur_fps=self.cur_fps, video_backend=self.data_args.video_backend)                    
                if video is None:
