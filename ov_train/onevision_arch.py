...
from streammind.model.multimodal_projector import load_mm_projector
from streammind.model.multimodal_encoder.builder import build_vision_tower
from streammind.model.multimodal_projector.builder import build_vision_projector
from streammind.mm_utils import get_anyres_image_grid_shape
from streammind.constants import NUM_FRAMES, IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN,DEFAULT_MMODAL_PATCH_TOKEN, DEFAULT_MMODAL_START_TOKEN, DEFAULT_MMODAL_END_TOKEN, MMODAL_TOKEN_INDEX
from torch.nn import BCEWithLogitsLoss, CrossEntropyLoss, MSELoss
from itertools import accumulate

class OneVisionStreamMetaModel:

    def __init__(self, config):
        super(OneVisionStreamMetaModel, self).__init__(config)
...
class OneVisionStreamMetaForCausalLM(ABC):
    ...
    def _encode_frames_with_onevision(self, videos, max_frames=None):
        ...
        if max_frames is not None and B * T > max_frames:
            if B != 1:
                raise ValueError(
                    "max_frames truncation requires B == 1 (original CLIP "
                    f"path was only called with single-video batches); got B={B}."
                )
        ...
        if H % factor != 0 or W % factor != 0:
            raise ValueError(
                f"Frame H={H}, W={W} must be divisible by patch_size*spatial_merge_size={factor}."
            )
        grid_h = H // patch_size
        grid_w = W // patch_size
        h_m = grid_h // merge_size
        w_m = grid_w // merge_size
        ...
        # Following the original StreamMind / VideoLLaMA2 design, the vision tower
    ...
    def encode_images_or_videos(self, images_or_videos, modalities):
        ...
        frames_features = self._encode_frames_with_onevision(videos)
    ...
    def encode_images_or_videos_score_cls_video_cls_autoregressive(self, images_or_videos,cls_inference = False,cls_training = False,caption_info = None,prompt_time_input_ids = None,prompt_time_lable = None):
        ...
        # if frames_features.dtype != torch.bfloat16:
        #     frames_features = frames_features.to(torch.bfloat16)
    ...
    def exponential_sampling(self, tensor, percentage=0.6):
        ...
        indices = torch.linspace(0, n - 1, num_samples).int().tolist()  # 线性间隔采样
        # print(n,num_samples,indices[-1])
    ...
    def prepare_inputs_labels_for_multimodal_score_stream(self, input_ids, attention_mask, past_key_values, labels, X_modalities,timestamp,sample_per=0.5,sample_type="all",**kwargs):
        ...
        Xs, keys = X_modalities
        exactor_output,feature_idx = self.encode_images_or_videos_score_cls_video_cls_autoregressive(Xs,
                                                                                                cls_training = cls_trainging,
                                                                                                cls_inference=cls_inference,
                                                                                                prompt_time_input_ids = input_ids,
                                                                                                prompt_time_lable = labels)#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        # import pdb
        # pdb.set_trace()
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while X_token_indices.numel() > 0:
                if sample_type == "log":
                    cur_X_features =self.exponential_sampling(X_features[0][start_feature_idx[cur_X_idx]:feature_idx[cur_X_idx]], sample_per)
                elif sample_type == "similarity" :
                    cur_X_features =self.similarity_sampling(X_features[0][start_feature_idx[cur_X_idx]:feature_idx[cur_X_idx]], sample_per)
                else:
    ...
    def prepare_inputs_labels_for_multimodal_score_stream_inference(
        ...
        # X_features, cls_feature ,frames_features = self.encode_images_or_videos_score_cls_inference(Xs,kwargs["frames_features"])#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        # if kwargs["past_review_caption"] is not None:
        #     X_features, cls_feature ,frames_features = self.encode_images_or_videos_score_cls_inference_allframe(Xs,kwargs["frames_features"],past_caption =self.get_model().embed_tokens(kwargs["past_review_caption"].to(Xs.device)))#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        # else:
        #     X_features, cls_feature ,frames_features = self.encode_images_or_videos_score_cls_inference_allframe(Xs,kwargs["frames_features"])#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        ...
        # pdb.set_trace()
        # # print(cls_feature)
        # if torch.randint(2,(1,1))[0].item() == 0:
        #     cls_feature = torch.tensor([[0,1]])
        # else:
        #     cls_feature = torch.tensor([[1,0]])
        #cls_feature:[eos, no eos]
        # import pdb
        # pdb.set_trace()
        softmax = nn.Softmax()
        cls_feature = softmax(cls_feature)
        # print(cls_feature)
        pred = cls_feature.argmax(dim=1).item()
        # pred = 1
        if pred == 0:
            #说明是eos:
            return None, None, None, None, None, pred, frames_features
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            while X_token_indices.numel() > 0:
                ...
                cur_new_input_embeds.append(self.get_model().embed_tokens(cur_input_ids[:X_token_start])) 
                if kwargs["past_review_caption"] is not None:
                    cur_new_input_embeds.append(self.get_model().embed_tokens(kwargs["past_review_caption"][0].to(cur_input_ids.device))) 
                cur_new_input_embeds.append(cur_X_features)
                if labels is not None:
                    cur_new_labels.append(cur_labels[:X_token_start])
                    cur_new_labels.append(torch.full((cur_X_features.shape[0],), IGNORE_INDEX, device=labels.device, dtype=labels.dtype))
        ...
        return None, attention_mask, past_key_values, new_input_embeds, new_labels,pred,None
    ...
    def prepare_inputs_labels_for_multimodal_score_stream_inference_demo(
        ...
        X_features, cls_feature ,frames_features,interval_id = self.encode_images_or_videos_score_cls_inference_allframe_demo(
            Xs,kwargs["frames_features"], frames_features_shape = kwargs["interval_id_list"])#len(Xs)=16,sx["image"]=[3,336,336],xs["video"]=[32,3,336,336] ,x_feature:[16,576,4096],keys表示的是Xs的modality：video or image,x_features:[16,576,4096]
        # import pdb
        # pdb.set_trace()
        softmax = nn.Softmax()
        cls_feature = softmax(cls_feature)
        ...
        # pred = (cls_feature[1] > 0.3).int()
        # pred = 1
        if pred == 0:
            #说明是eos:
            return None, None, None, None, None, pred, frames_features, interval_id
        else:
            kwargs["interval_id_list"].append(interval_id)#记录说话的位置
